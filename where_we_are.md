...
- Windows Claude Code config parity: `scripts/windows-claude-setup.ps1` brings a fresh Windows install in line with the Mac config (theme, auto mode, permissions, ping sounds, statusline). Idempotent. After running, `/plugin install clangd-lsp@claude-plugins-official` inside Claude Code completes the plugin parity
